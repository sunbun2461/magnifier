...
class EnhancedImageMagnifier {
	constructor(selector, options = {}) {
		...
		this.options = {
			...
			// Interaction timing
			fadeDurationInMs: options.fadeDurationInMs || 150,  // Reduced from 300ms for snappier response
	...
	onPointerEnter(e) {
		...
		this.timers.hoverDelay = setTimeout(() => {
			this.activateMagnifier(e);
		}, this.options.hoverDelayInMs);
	}
	...
	activateMagnifier(e) {
		...
		this.elements.magnifierWindow.offsetHeight;
	...
	updateMagnifiedImagePosition(e) {
		...
		if (this.options.mode === 'move') {
			...
			// Zoom-on-hover mode: position the scaled image
			...
			// Calculate the ideal position (centered on cursor point)
			let left = -(xPercent * magnifiedImageWidth) + (windowWidth / 2);
			let top = -(yPercent * magnifiedImageHeight) + (windowHeight / 2);

			// Calculate the bounds to prevent white space - completely lock the image
			// The magnified image should never be positioned such that white space shows
			const maxLeft = 0; // Image's left edge aligns with window's left edge
			const maxTop = 0; // Image's top edge aligns with window's top edge
			const minLeft = windowWidth - magnifiedImageWidth; // Image's right edge aligns with window's right edge
			const minTop = windowHeight - magnifiedImageHeight; // Image's bottom edge aligns with window's bottom edge

			// Completely lock position to prevent ANY white space - no buffer
